...
stack_elem_t *stack_append(stack_t *stack, stack_elem_t *stack_elem) {
    if (stack_elem == NULL)
        stack_elem = elem_create(NULL, NULL);
...
void stack_test() {
    ...
    for (int i = 0; i < 1000000; ++i) {
        stack_append(stack, elem_create("s1", NULL));
        stack_append(stack, elem_create("s2", NULL));
        stack_append(stack, elem_create("s3", NULL));
        stack_append(stack, elem_create("s4", NULL));
